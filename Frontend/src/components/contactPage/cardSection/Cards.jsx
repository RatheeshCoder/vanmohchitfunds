...
const Cards = () => {
  return (
    <>
      <div className="relative grid min-h-screen grid-cols-1 gap-10 px-10 py-6 overflow-hidden md:grid-cols-2 lg:grid-cols-3 sm:py-12">
        {BranchData.map((branch) => (
          <>
            <div className="relative justify-around px-6 pt-10 pb-8 overflow-hidden transition-all duration-300 bg-white shadow-xl cursor-pointer group ring-1 ring-gray-900/5 hover:-translate-y-1 hover:shadow-2xl sm:mx-auto sm:max-w-sm sm:rounded-lg sm:px-10">
              ...
              <div className="relative z-10 max-w-md mx-auto">
                ...
                <div
                  className="pt-5 space-y-6 text-base leading-7 text-gray-600 transition-all duration-300 group-hover:text-white/90 text-sm md:text-base text-lg"
                  key={branch.id}
                >
                  <p>{branch.address}</p>
                </div>

                <div className="pt-5 text-base font-semibold leading-7 text-sky-50">
                  <a
                    href={branch.link}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    <button className="text-gray-600 group-hover:text-white/90">
